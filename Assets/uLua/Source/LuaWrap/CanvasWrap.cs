...
public class CanvasWrap
{
	public static LuaMethod[] regs = new LuaMethod[]
	{
		...
		new LuaMethod("GetETC1SupportedCanvasMaterial", GetETC1SupportedCanvasMaterial),
	...
	static LuaField[] fields = new LuaField[]
	{
		...
		new LuaField("rootCanvas", get_rootCanvas, null),
	};
	...
	{
		LuaScriptMgr.Push(L, typeof(Canvas));
		return 1;
	}

	public static void Register(IntPtr L)
	{
		LuaScriptMgr.RegisterLib(L, "UnityEngine.Canvas", typeof(Canvas), regs, fields, typeof(UnityEngine.Behaviour));
	}
	...
	static int get_rootCanvas(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name rootCanvas");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index rootCanvas on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.rootCanvas);
		return 1;
	}
	...
	static int set_renderMode(IntPtr L)
	{
		...
		obj.renderMode = LuaScriptMgr.GetNetObject<RenderMode>(L, 3);
		return 0;
	}
	...
	static int set_worldCamera(IntPtr L)
	{
		...
		obj.worldCamera = LuaScriptMgr.GetUnityObject<Camera>(L, 3);
		return 0;
	}
	...
	{
		LuaScriptMgr.CheckArgsCount(L, 0);
		Material o = Canvas.GetETC1SupportedCanvasMaterial();
		LuaScriptMgr.Push(L, o);
		return 1;
	}
