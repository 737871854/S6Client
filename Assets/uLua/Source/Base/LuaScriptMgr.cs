...
public class LuaScriptMgr
{
    ...
    {
        byte[] str = null;
        string path = UtilCommon.LuaPath(name);
	...
	/*		ResourceMisc.AssetWrapper aw = ioo.resourceManager.LoadAsset(path, typeof(TextAsset));
			TextAsset ta = aw.GetAsset() as TextAsset;
            str = ta.bytes;
			//str = new byte[ta.bytes.Length];
			//ta.bytes.CopyTo(str, 0);
			ioo.resourceManager.ReleaseAsset(aw);*/
        }
        catch
        {
    ...
    static bool PushLuaFunction(IntPtr L, string fullPath)
    {
        ...
        if (pos > 0)
        {
            ...
            if (PushLuaTable(L, tableName))
            {
                ...
                Debugger.LogError("Push lua function {0} failed", fullPath);
                LuaDLL.lua_settop(L, oldTop);
                return false;
            }
    ...
    private static UnityEngine.Object GetUnityObject(IntPtr L, int stackPos, Type type)
    {
	...
	public static void PushArrayEx(IntPtr L, System.Array o)
	{
		LuaDLL.lua_newtable (L);
		for (int i = 0; i < o.GetLength(0); ++i)
		{
			LuaDLL.lua_pushnumber(L, i+1);
			object obj = o.GetValue(i);
			Type t = obj.GetType();
			if (t.IsValueType)
			{
				if (t == typeof(bool))
			...
			else
			{
				if (t.IsArray)
				{
					PushArrayEx(L, (System.Array)obj);
				}
				...
				else if (t.IsSubclassOf(typeof(UnityEngine.Object)))
				{
					...
					{
						GetTranslator(L).pushObject(L, obj, "luaNet_metatable");                    
					}
				}
				...
				{
					((LuaFunction)obj).push(L);
				}
				else if (t == typeof(LuaCSFunction))
				{
					GetTranslator(L).pushFunction(L, (LuaCSFunction)obj);
				}
